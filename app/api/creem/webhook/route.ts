...
export async function POST(request: NextRequest) {
  ...
  if (!verifyCreemWebhookSignature(payloadText, signature)) {
    return NextResponse.json({ error: 'Invalid Creem signature.' }, { status: 401 });
  }
  ...
  if (!eventType || !object) {
    return NextResponse.json({ error: 'Malformed Creem webhook event.' }, { status: 400 });
  }
  ...
  try {
    ...
    const plan =
      getBillingPlanByPriceId(String(metadata.priceId || '')) ||
    ...
    const result = await processBillingWebhookEvent({
      source: 'creem',
      eventId: String(event.id || `creem_event_${Date.now()}`),
      eventType,
      ...
      stripeSubscriptionId: getSubscriptionId(subscriptionValue),
      status: getStatusFromEvent(eventType, subscriptionValue, plan.priceId),
