...
| BILL155-PRICING-MODEL | Billing 1.5.5 | 1.5.5 / Pricing V2 Model | Public pricing and internal plan semantics must move to the `Single Review / Starter / Plus / Family` ladder with explicit seat, subject-slot, and review-credit limits. |
| BILL155-PLAN-MAPPING | Billing 1.5.5 | 1.5.5 / Provider Mapping | Checkout and provider integrations must map plan code separately from billing cadence, with distinct monthly and annual price ids for recurring tiers. |
| BILL155-ENTITLEMENT-SNAPSHOT | Billing 1.5.5 | 1.5.5 / Billing Snapshot | Local entitlement snapshots must expose active plan code, billing mode, seat limits, subject-slot limits, review-credit state, and add-on guidance for the billing center. |
| BILL155-PUBLIC-CONSISTENCY | Billing 1.5.5 | 1.5.5 / Public Consistency | `/pricing` and `/dashboard/billing` must present the same pricing-v2 language and remove the legacy `One-Time Diagnosis / Parent Weekly / Parent Annual` ladder. |
